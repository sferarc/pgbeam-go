...
type AuditChainVerification struct {
	...
	// AnchorVerifiedFromSeq First sequence number covered by a signed checkpoint that was re-derived and matched during this verification. Together with anchor_verified_through_seq this is how far back the anchor's coverage actually reaches, which after retention is less than the whole chain. Null when no signed range could be checked.
	AnchorVerifiedFromSeq *int64 `json:"anchor_verified_from_seq,omitempty"`

	// AnchorVerifiedThroughSeq Last sequence number covered by a signed checkpoint that was re-derived and matched during this verification. Null when none could be.
	AnchorVerifiedThroughSeq *int64 `json:"anchor_verified_through_seq,omitempty"`
	...
	// LinkVerifiedFromSeq First sequence number whose prev_hash was actually checked against a known predecessor. Equal to chain_start_seq on a chain whose floor is verified, and one above it when link_status is predecessor_expired, so a caller can tell what was checked from what was assumed. Links are checked from here through chain_end_seq. Null when none could be.
	LinkVerifiedFromSeq *int64 `json:"link_verified_from_seq,omitempty"`

	// Ok True when no tampering or deletion was detected: every chained entry in the range recomputes to its stored hash, and every link that could be checked matched. Two links may be uncheckable rather than wrong, and both leave ok true and say so on their own field: the one below the first checked entry when retention has deleted what it follows (link_status), and a signed checkpoint whose range retention has deleted (anchor_status). Read them alongside ok to know how much a pass covers.
	Ok bool `json:"ok"`
...
// PolicyProfileInputContentScanMode Result-content scanning, enforced on the data-plane relay path: values on their way out to an agent are checked for instruction-shaped content (stored prompt injection). off scans nothing and costs nothing. annotate scans and forwards every value unchanged, recording what it found. block is accepted and scans exactly as annotate does, but refusing the statement is not implemented yet, so today it also forwards every value. No mode drops or rewrites a row. A flagged statement records one content_flagged audit entry naming the columns, the techniques and whether the scan covered the whole result, alongside the proxy's own metrics and logs; the entry carries no value from the row itself, and the anomaly feed and the webhook stream are not wired to it yet. Values the scan cannot read (binary-format columns, columns this profile masks, and anything past content_scan_max_bytes) are counted as unscanned rather than reported clean. A proxy build older than result-content scanning ignores this field.
